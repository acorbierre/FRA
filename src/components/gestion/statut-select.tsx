...
const COLORS: Record<string, { bg: string; text: string }> = {
  ...
  'Envoyée au CS':       { bg: 'bg-amber-100',   text: 'text-amber-800' },
  'En évaluation':       { bg: 'bg-amber-200',   text: 'text-amber-900' },
  'En délibération CS':  { bg: 'bg-orange-100',  text: 'text-orange-800' },
...
export default function StatutSelect({ id, statut, options, endpoint, labels }: Props) {
  ...
  return (
    <div ref={ref} className="relative inline-block">
      <button
        onClick={() => setOpen(o => !o)}
        disabled={pending}
        className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-opacity disabled:opacity-50 ${colors.bg} ${colors.text}`}
      >
        {labels?.[value] ?? value}
        ...
        <div className="absolute z-50 left-0 top-full mt-1 w-44 rounded-lg border border-border bg-background shadow-md py-1">
          {options.map(opt => {
            ...
            return (
              ...
              >
                <span className={`px-2 py-0.5 rounded-full font-medium ${c.bg} ${c.text}`}>{labels?.[opt] ?? opt}</span>
              </button>
            )
          })}
        </div>
